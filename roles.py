...
async def update_roles(ctx, score):
    async def add_role_by_name(name):
        await ctx.author.add_roles([r for r in roles if r.name == name][0].id)
    ...
    for role in guild.roles:
        if role.name in config.levels.names + config.levels.rank_names:
            await ctx.author.remove_roles(role.id)

    # Find the correct level role
    ptidx = bisect.bisect_right(config.levels.points, score.total_points)
